...
import {
  ...
  CheckCircle2, AlertCircle, Clock, Eye as ViewsIcon, User, Tag
} from 'lucide-react';
...
export default function AdminBlog() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backendUrl}/api/admin/blog?${queryParams.toString()}`, {
        method: 'GET',
      ...
      if (data.success) {
        ...
        setError(data.message || 'Failed to retrieve blog posts.');
      }
    } catch (err) {
  ...
  const handleToggleStatus = async (post) => {
    const newStatus = post.is_active ? 0 : 1;
    const actionName = newStatus === 1 ? 'publish' : 'unpublish';

    if (!window.confirm(`Are you sure you want to ${actionName} post "${post.title}"?`)) {
      return;
    }

    try {
      const backendUrl = window.location.hostname === 'localhost' ? 'http://localhost:5000' : '';
      const res = await fetch(`${backendUrl}/api/admin/blog/${post.id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      const data = await res.json();
      if (data.success) {
        setActionSuccess(data.message || `Post successfully ${newStatus === 1 ? 'published' : 'unpublished'}.`);
  ...
  const handleDelete = async (post) => {
    ...
    try {
      const backendUrl = window.location.hostname === 'localhost' ? 'http://localhost:5000' : '';
      const res = await fetch(`${backendUrl}/api/admin/blog/${post.id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include'
      });
      const data = await res.json();
      if (data.success) {
        setActionSuccess(data.message || 'Post deleted successfully.');
        setTimeout(() => setActionSuccess(null), 4000);
        fetchPosts();
      } else {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#FFFFFF', letterSpacing: '-0.02em' }}>
            WordPress-Like Blog CMS
          </h1>
          <p style={{ color: '#94A3B8', fontSize: '0.86rem', marginTop: '4px' }}>
            Create and edit technical articles, tech news, tutorials, SEO meta tags, and publication statuses.
          </p>
        </div>
      ...
      <div style={{ background: '#0B1120', border: '1px solid var(--border-light)', borderRadius: '12px', padding: '16px' }}>
        <form onSubmit={handleSearchSubmit} style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '240px' }}>
            ...
            <input
              type="text"
              placeholder="Search by article title, slug, or author..."
              value={searchQuery}
      ...
      <div style={{ background: '#0B1120', border: '1px solid var(--border-light)', borderRadius: '12px', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '40px', textAlign: 'center', color: '#64748B' }}>
            ...
            <p style={{ fontWeight: 600 }}>No blog posts found matching search filter.</p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'left', minWidth: '650px' }}>
              <thead>
                <tr style={{ background: '#070A12', color: '#64748B', borderBottom: '1px solid var(--border-light)', textTransform: 'uppercase', fontSize: '0.72rem', letterSpacing: '0.05em' }}>
                  ...
                  <th style={{ padding: '12px' }}>Category & Author</th>
                    ...
                    <td style={{ padding: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {p.featured_image || p.image ? (
                          ...
                          <div style={{ width: '48px', height: '36px', borderRadius: '6px', background: '#070A12', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#00B4D8' }}>
                            <FileText size={18} />
                          </div>
                        )}
                        <div style={{ maxWidth: '300px' }}>
                          ...
                          <div style={{ fontSize: '0.74rem', color: '#64748B' }}>/{p.slug}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '12px' }}>
                      <div style={{ color: '#CBD5E1', fontWeight: 600 }}>{p.category || 'Tech & Engineering'}</div>
                      <div style={{ fontSize: '0.74rem', color: '#64748B' }}>{p.author || 'Editorial'}</div>
                    </td>
                    <td style={{ padding: '12px', color: '#94A3B8', fontSize: '0.8rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <ViewsIcon size={14} color="#00B4D8" /> {p.views_count || 0}
                      </div>
                    </td>
                    <td style={{ padding: '12px' }}>
                      <span style={{
                    ...
                    <td style={{ padding: '12px', textAlign: 'right' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '6px' }}>
                        <button
                          ...
                          title={p.is_active ? "Unpublish Post" : "Publish Post"}
                        ...
                        <button
                          ...
                          title="Edit Post in WordPress Editor"
