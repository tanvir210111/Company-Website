...
import {
  GraduationCap, Search, Filter, RefreshCw, Eye, CheckCircle2, XCircle, ChevronLeft, ChevronRight, UserCheck, UserX
} from 'lucide-react';
...
export default function AdminStudents() {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backendUrl}/api/admin/students?${queryParams.toString()}`, {
        method: 'GET',
      ...
      if (data.success) {
        ...
        setError(data.message || 'Failed to retrieve student accounts.');
      }
    } catch (err) {
  ...
  const handleToggleStatus = async (student) => {
    const targetStatus = student.is_active ? 0 : 1;
    const actionText = targetStatus === 1 ? 'activate' : 'deactivate';

    if (!window.confirm(`Are you sure you want to ${actionText} student account "${student.full_name || student.name}"?`)) {
      return;
    }

    try {
      const backendUrl = window.location.hostname === 'localhost' ? 'http://localhost:5000' : '';
      const res = await fetch(`${backendUrl}/api/admin/students/${student.id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      const data = await res.json();
      if (data.success) {
        setActionSuccess(data.message || `Student account ${actionText}d.`);
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#FFFFFF', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <GraduationCap color="#00B4D8" size={28} /> Student Management
          </h1>
          <p style={{ color: '#94A3B8', fontSize: '0.86rem', marginTop: '4px' }}>
            View, inspect, and manage registered student accounts, course enrollments, and profiles.
          </p>
        </div>
      </div>
      ...
      <div style={{ background: '#0B1120', border: '1px solid var(--border-light)', borderRadius: '12px', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '60px', textAlign: 'center', color: '#64748B' }}>
            <GraduationCap size={40} style={{ margin: '0 auto 12px auto', opacity: 0.5 }} />
            <p style={{ fontWeight: 600 }}>No student accounts found.</p>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.86rem' }}>
            <thead>
              <tr style={{ background: '#070A12', color: '#64748B', borderBottom: '1px solid var(--border-light)' }}>
                ...
                <th style={{ padding: '14px 16px' }}>Education / Degree</th>
                <th style={{ padding: '14px 16px' }}>Enrollments</th>
              ...
              {students.map(s => (
                <tr key={s.id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)' }}>
                  ...
                  <td style={{ padding: '14px 16px', color: '#CBD5E1' }}>
                    {s.education_level || 'General Student'}
                  </td>
                  <td style={{ padding: '14px 16px', color: '#00B4D8', fontWeight: 700 }}>
                    {s.enrollments_count || 0} Courses
                  </td>
                  ...
                  <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                    <div style={{ display: 'inline-flex', gap: '8px' }}>
                      <button
                        onClick={() => { setSelectedStudentId(s.id); setProfileModalOpen(true); }}
                        style={{
                      ...
                      <button
                        ...
                        style={{
                          background: s.is_active ? 'rgba(239, 68, 68, 0.15)' : 'rgba(16, 185, 129, 0.15)',
                          color: s.is_active ? '#EF4444' : '#10B981',
                          border: s.is_active ? '1px solid rgba(239, 68, 68, 0.3)' : '1px solid rgba(16, 185, 129, 0.3)',
                          borderRadius: '6px',
                          padding: '6px 10px',
                          fontSize: '0.76rem',
                          fontWeight: 700,
                        ...
                        title={s.is_active ? 'Deactivate Student Account' : 'Activate Student Account'}
                      >
