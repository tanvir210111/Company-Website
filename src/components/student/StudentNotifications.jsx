...
export default function StudentNotifications() {
  ...
  const fetchNotifications = async () => {
    ...
    setError(null);
    try {
      const res = await adminFetch('/api/student/notifications');
  ...
  const handleMarkAsRead = async (id) => {
    try {
      await adminFetch(`/api/student/notifications/${id}/read`, { method: 'PATCH' });
  ...
  const handleMarkAllRead = async () => {
    try {
      await adminFetch('/api/student/notifications/read-all', { method: 'POST' });
  ...
  if (loading) {
    return (
      <div style={{ padding: '80px 20px', textAlign: 'center', color: '#94A3B8' }}>
        ...
        <p style={{ fontWeight: 600, fontSize: '0.95rem' }}>Loading Notifications...</p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#FFFFFF', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Bell size={24} color="#00B4D8" /> Notifications & Academic Alerts
          </h1>
          <p style={{ color: '#94A3B8', fontSize: '0.85rem' }}>
            Updates regarding batch schedules, payment receipts, assignments, and institute notices.
          </p>
        </div>
      ...
      {notifications.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: '0.88rem' }}>You're all caught up! No unread notifications or alerts.</p>
        </div>
      ) : (
          ...
          {notifications.map(n => (
            <div
              ...
              style={{
                background: '#0B1120',
                borderRadius: '10px',
                border: `1px solid ${n.is_read ? 'rgba(255, 255, 255, 0.06)' : 'rgba(0, 180, 216, 0.3)'}`,
                padding: '16px 20px',
                display: 'flex',
            ...
            >
              <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                <div style={{
                  ...
                  height: '10px',
                  borderRadius: '50%',
                  background: n.is_read ? 'transparent' : '#00B4D8',
                  marginTop: '6px',
                  flexShrink: 0
                }} />
                <div>
