...
export default function StudentPayments() {
  ...
  const fetchPayments = async () => {
    ...
    setError(null);
    try {
      const res = await adminFetch('/api/student/payments');
      ...
      setError('Could not connect to payment records service.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ padding: '80px 20px', textAlign: 'center', color: '#94A3B8' }}>
        ...
        <p style={{ fontWeight: 600, fontSize: '0.95rem' }}>Loading Payment & Transaction Records...</p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#FFFFFF', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <CreditCard size={24} color="#10B981" /> Payments & Official Receipts
          </h1>
          <p style={{ color: '#94A3B8', fontSize: '0.85rem' }}>
            Verified SSLCommerz, Bkash, Nagad, and bank payment transactions with official invoices.
          </p>
        </div>
        ...
        <div style={{
          ...
          Total Verified Paid: ৳{totalPaid.toLocaleString()}
        </div>
      </div>
      ...
      {payments.length === 0 ? (
        <div style={{
          ...
          <h3 style={{ color: '#FFFFFF', fontWeight: 700, marginBottom: '6px' }}>No Payment History Available</h3>
          <p style={{ fontSize: '0.88rem' }}>You have not completed any online or manual payment transactions yet.</p>
        </div>
      ) : (
        <div style={{
          background: '#0B1120',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '700px' }}>
            <thead>
              <tr style={{ background: 'rgba(255, 255, 255, 0.02)', borderBottom: '1px solid rgba(255, 255, 255, 0.08)' }}>
                <th style={{ padding: '14px 16px', fontSize: '0.75rem', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase' }}>Order ID</th>
                <th style={{ padding: '14px 16px', fontSize: '0.75rem', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase' }}>Course Program</th>
                <th style={{ padding: '14px 16px', fontSize: '0.75rem', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase' }}>Amount</th>
                <th style={{ padding: '14px 16px', fontSize: '0.75rem', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase' }}>Gateway</th>
                <th style={{ padding: '14px 16px', fontSize: '0.75rem', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase' }}>Date</th>
              ...
              {payments.map(p => (
                <tr key={p.id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.04)' }}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: '0.86rem', fontWeight: 700, color: '#FFFFFF' }}>
                    {p.course_title || (p.enrollment_no ? `Enrollment #${p.enrollment_no}` : 'IT Training Tuition')}
                  </td>
