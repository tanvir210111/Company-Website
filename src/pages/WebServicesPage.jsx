...
import { ArrowLeft, Monitor, CheckCircle, Briefcase, Star, ArrowRight } from 'lucide-react';

export default function WebServicesPage({ onNavigate, onOpenQuote }) {
  const webServices = SERVICES.filter(s => s.category === "Website Design" || s.title.toLowerCase().includes('website') || s.title.toLowerCase().includes('ecommerce') || s.title.toLowerCase().includes('wordpress'));
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ color: '#00B4D8', fontWeight: 700, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Monitor size={18} color="#00B4D8" /> Web Engineering & E-Commerce Division
          </div>
          ...
          <p style={{ color: '#94A3B8', fontSize: '1.1rem', maxWidth: '850px', lineHeight: 1.6 }}>
            Custom static and dynamic web applications, e-commerce stores, resort portals, WordPress theme engineering, speed optimization, and 24/7 web maintenance.
          </p>
        </div>

        {/* Services Grid with Equal Card Heights & Related Photos */}
          ...
          {webServices.map(service => (
            <div key={service.id} style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                <div style={{ position: 'relative', height: '190px', width: '100%', overflow: 'hidden', background: '#0B1120', borderBottom: '2px solid #00B4D8' }}>
                  <img 
                    src={service.image_url || service.image || "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=800&q=80"} 
                    alt={service.title} 
                    onError={(e) => {
                  ...
                  />
                  <span style={{ position: 'absolute', top: '14px', right: '14px', background: 'rgba(7, 10, 18, 0.85)', backdropFilter: 'blur(8px)', color: '#00B4D8', fontSize: '0.75rem', fontWeight: 700, padding: '4px 12px', borderRadius: '20px', border: '1px solid var(--border-light)', zIndex: 10 }}>
                    {service.category}
                  </span>
                </div>
              ...
              <div style={{ padding: '16px 22px 20px 22px', borderTop: '1px solid var(--border-light)', marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <div style={{ fontSize: '0.78rem', color: '#94A3B8' }}>Starting Investment</div>
                  <div style={{ fontSize: '1.15rem', fontWeight: 800, color: '#00B4D8' }}>
        ...
        <div style={{ textAlign: 'center', background: 'linear-gradient(135deg, #0F172A, #1E293B)', padding: '50px 30px', borderRadius: '24px', border: '1px solid var(--border-light)' }}>
          <h3 style={{ fontSize: '1.8rem', color: '#FFFFFF', fontWeight: 800, marginBottom: '12px' }}>Build Your Enterprise Web Presence</h3>
          <p style={{ color: '#94A3B8', marginBottom: '28px', maxWidth: '600px', margin: '0 auto 28px auto' }}>Talk to our senior web architects for a customized project estimation.</p>
          <button onClick={() => onOpenQuote()} className="btn-primary" style={{ padding: '14px 32px', fontSize: '1.05rem' }}>
            <Briefcase size={20} /> Request Custom Web Proposal
          </button>
        </div>
      </div>
    </div>
