...
import { ArrowLeft, Clock, Calendar, Star, Users, CheckCircle, GraduationCap, Terminal, ArrowRight } from 'lucide-react';

export default function ProgrammingCoursesPage({ onNavigate, onOpenAdmission }) {
  const [activeCourseModal, setActiveCourseModal] = useState(null);

  const progCourses = COURSES.filter(c => c.category === "Programming & Software" || c.title.toLowerCase().includes('c') || c.title.toLowerCase().includes('python') || c.title.toLowerCase().includes('java'));
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ color: '#00B4D8', fontWeight: 700, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Terminal size={18} color="#00B4D8" /> Department of Programming Languages
          </div>
          ...
          <p style={{ color: '#94A3B8', fontSize: '1.1rem', maxWidth: '850px', lineHeight: 1.6 }}>
            Master computer science logic, algorithms, Data Structures, C Programming, Object-Oriented C++, Java, and Python Artificial Intelligence/Machine Learning.
          </p>
        </div>
              ...
              <div className="course-body">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', color: '#FFB703', fontWeight: 700 }}>
                    <Star size={14} fill="#FFB703" /> {course.rating} (130+ Reviews)
                  </div>
        ...
        <div style={{ textAlign: 'center', background: 'linear-gradient(135deg, #0F172A, #1E293B)', padding: '50px 30px', borderRadius: '24px', border: '1px solid var(--border-light)' }}>
          <h3 style={{ fontSize: '1.8rem', color: '#FFFFFF', fontWeight: 800, marginBottom: '12px' }}>Build Strong Algorithmic Fundamentals</h3>
          <p style={{ color: '#94A3B8', marginBottom: '28px', maxWidth: '600px', margin: '0 auto 28px auto' }}>Learn C, C++, Java & Python with 100% lab exercises.</p>
          <button onClick={() => onOpenAdmission()} className="btn-primary" style={{ padding: '14px 32px', fontSize: '1.05rem' }}>
            <GraduationCap size={20} /> Enroll in Programming Batch
          </button>
        </div>
      </div>
