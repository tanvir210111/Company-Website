...
import { ArrowLeft, Clock, Calendar, Star, Users, CheckCircle, GraduationCap, Palette, ArrowRight } from 'lucide-react';

export default function GraphicsCoursesPage({ onNavigate, onOpenAdmission }) {
  const [activeCourseModal, setActiveCourseModal] = useState(null);

  const graphicsCourses = COURSES.filter(c => c.category === "Graphics & UI/UX" || c.title.toLowerCase().includes('graphics') || c.title.toLowerCase().includes('ux') || c.title.toLowerCase().includes('autocad'));
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ color: '#FF6B00', fontWeight: 700, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Palette size={18} color="#FF6B00" /> Department of Graphics & Visual Design
          </div>
          <h1 style={{ fontSize: '2.8rem', fontWeight: 800, color: '#FFFFFF', marginBottom: '16px', lineHeight: 1.15 }}>
            Graphics & UI/UX Design Courses
          </h1>
          <p style={{ color: '#94A3B8', fontSize: '1.1rem', maxWidth: '850px', lineHeight: 1.6 }}>
            Master Adobe Illustrator, Photoshop, Figma UI/UX research, AutoCAD 2D 3D, Interior Design, and Premiere Pro Video Editing.
          </p>
        </div>

        {/* Courses Grid */}
        <div className="courses-grid" style={{ marginBottom: '60px' }}>
          {graphicsCourses.map(course => (
              ...
              <div className="course-body">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', color: '#FFB703', fontWeight: 700 }}>
                    <Star size={14} fill="#FFB703" /> {course.rating} (110+ Reviews)
                  </div>
        ...
        <div style={{ textAlign: 'center', background: 'linear-gradient(135deg, #0F172A, #1E293B)', padding: '50px 30px', borderRadius: '24px', border: '1px solid var(--border-light)' }}>
          <h3 style={{ fontSize: '1.8rem', color: '#FFFFFF', fontWeight: 800, marginBottom: '12px' }}>Unleash Your Creative Potential</h3>
          <p style={{ color: '#94A3B8', marginBottom: '28px', maxWidth: '600px', margin: '0 auto 28px auto' }}>Learn Figma, Illustrator & Photoshop from award-winning mentors.</p>
          <button onClick={() => onOpenAdmission()} className="btn-primary" style={{ padding: '14px 32px', fontSize: '1.05rem' }}>
            <GraduationCap size={20} /> Enroll in Graphics Design Batch
          </button>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setActiveCourseModal(null)}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background: '#0B1120', padding: '18px', borderRadius: '12px', border: '1px solid var(--border-light)', marginBottom: '20px' }}>
              <h4 style={{ color: '#FF6B00', marginBottom: '10px' }}>Curriculum Modules:</h4>
