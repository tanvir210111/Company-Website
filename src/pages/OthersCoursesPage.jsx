...
import { ArrowLeft, Clock, Calendar, Star, Users, CheckCircle, GraduationCap, Laptop, ArrowRight } from 'lucide-react';

export default function OthersCoursesPage({ onNavigate, onOpenAdmission }) {
  const [activeCourseModal, setActiveCourseModal] = useState(null);

  const otherCourses = COURSES.filter(c => c.category === "Others & Skills" || c.title.toLowerCase().includes('freelance') || c.title.toLowerCase().includes('office') || c.title.toLowerCase().includes('excel'));
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ color: '#FF6B00', fontWeight: 700, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Laptop size={18} color="#FF6B00" /> Department of Freelancing & Office Productivity
          </div>
          <h1 style={{ fontSize: '2.8rem', fontWeight: 800, color: '#FFFFFF', marginBottom: '16px', lineHeight: 1.15 }}>
            Freelancing & Professional Skills Courses
          </h1>
          <p style={{ color: '#94A3B8', fontSize: '1.1rem', maxWidth: '850px', lineHeight: 1.6 }}>
            Master Upwork and Fiverr freelancing order hunting, Microsoft Office Suite management, and Advanced Excel financial modeling.
          </p>
        </div>
              ...
              <div className="course-body">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.85rem', color: '#FFB703', fontWeight: 700 }}>
                    <Star size={14} fill="#FFB703" /> {course.rating} (150+ Reviews)
                  </div>
        ...
        <div style={{ textAlign: 'center', background: 'linear-gradient(135deg, #0F172A, #1E293B)', padding: '50px 30px', borderRadius: '24px', border: '1px solid var(--border-light)' }}>
          <h3 style={{ fontSize: '1.8rem', color: '#FFFFFF', fontWeight: 800, marginBottom: '12px' }}>Start Earning Remotely from BD</h3>
          <p style={{ color: '#94A3B8', marginBottom: '28px', maxWidth: '600px', margin: '0 auto 28px auto' }}>Learn Upwork, Fiverr & Payoneer global payments step by step.</p>
          <button onClick={() => onOpenAdmission()} className="btn-primary" style={{ padding: '14px 32px', fontSize: '1.05rem' }}>
            <GraduationCap size={20} /> Enroll in Freelancing Masterclass
          </button>
        </div>
      </div>
